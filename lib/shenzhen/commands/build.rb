...
command :build do |c|
  ...
  c.action do |args, options|
    ...
    @identity_arg = %{-s "#{@identity}"}

    log "xcrun", "PackageApplication"
    abort unless system %{xcrun -sdk #{@sdk} PackageApplication -v "#{@app_path}" -o "#{@ipa_path}" --embed "#{options.embed || @dsym_path}" #{@identity_arg if @identity} #{'1> /dev/null' unless $verbose}}
